...
const AddProof = () => {
  ...
  const handleStatusChange = async (deliveryId, newStatus) => {
    try {
      await fetch(`https://backend-rho-three-58.vercel.app/delivery-status/${deliveryId}`, {
        method: "PUT",
        headers: {
      ...
      if (file) {
        ...
        try {
          const res = await axios.post(
            `https://backend-rho-three-58.vercel.app/upload-signature/${deliveryId}`,
            formData,
            {
              headers: {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post(
        `https://backend-rho-three-58.vercel.app/deliveries/${deliveryId}/proof`, // Your API endpoint
        formData,
        {
          headers: {
